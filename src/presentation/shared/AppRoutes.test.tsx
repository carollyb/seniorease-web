...
describe('SeniorEase route integration', () => {
  ...
  it('wraps profile and settings pages in one shared navigation landmark', () => {
    ...
    expect(
      screen.getAllByRole('navigation', { name: 'SeniorEase' }),
    ).toHaveLength(1);
  ...
  it('uses an accessible modal before completing an activity', async () => {
    ...
    expect(within(modal).getByText('Título:')).not.toBeNull();
    expect(within(modal).getByText('Enviar trabalho')).not.toBeNull();
    expect(within(modal).getByText('Lembrete:')).not.toBeNull();
